...
import { commitEmailChangeInFirestore } from "@/lib/auth";
  ...
  useEffect(() => {
    ...
    const processAction = async () => {
      ...
      try {
        ...
        // Step B: Check Firestore emailChangeRequests if this is an email change request
        if (rawReqId) {
          try {
            const reqSnap = await getDoc(doc(db, "emailChangeRequests", rawReqId));
            if (reqSnap.exists()) {
              activeReqDoc = reqSnap.data() as EmailChangeRequest;
            }
          } catch (reqErr) {
        ...
        // If not found by reqId, query by userId or email
        if (!activeReqDoc && (rawUid || auth.currentUser?.uid)) {
          ...
          try {
            ...
            if (!qSnap.empty) {
              // Find the newest request matching this target email or newest created
              const sorted = qSnap.docs
                .map((d) => d.data() as EmailChangeRequest)
        ...
        // Step C: Validate against Firestore Request Lifecycle Rules
        if (activeReqDoc) {
          ...
          if (activeReqDoc.status === "cancelled") {
            ...
            setStatus("cancelled");
            setErrorMessage("This email change request was cancelled. This verification link is no longer valid.");
            return;
          }
          ...
          if (activeReqDoc.status === "superseded") {
            ...
            setErrorMessage("This verification link has been replaced by a newer verification request. Please use the latest link sent to your email.");
            return;
          }

          // Check 3: 60-Second TTL Expiration
          if (activeReqDoc.status === "expired" || now > activeReqDoc.expiresAt) {
            console.warn("[EMAIL ACTION] Request has expired (60-second TTL elapsed). Aborting applyActionCode.");
            // Mark expired in Firestore if needed
            if (activeReqDoc.status !== "expired") {
              await setDoc(
        ...
        const targetUid = activeReqDoc?.userId || rawUid || auth.currentUser?.uid || "";
        ...
        // Step E: Set request status to verified in Firestore & commit profile update
        if (activeReqDoc) {
          try {
            await setDoc(
              doc(db, "emailChangeRequests", activeReqDoc.requestId),
        ...
        if (targetUid && targetNewEmail) {
          try {
            await commitEmailChangeInFirestore(targetUid, targetNewEmail, targetOldEmail);
            console.log("[EMAIL ACTION] Firestore profile synchronized to new verified email:", targetNewEmail);
          } catch (syncErr) {
            console.warn("[EMAIL ACTION] Notice: error committing email change in Firestore:", syncErr);
          }
        }
        ...
        {status === "success" && (
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Return to LinkCloud app tab and click &apos;Refresh Verification Status&apos;.
              </p>
            </div>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground max-w-md mx-auto leading-relaxed">
                This verification link has expired. LinkCloud email verification links are valid for 60 seconds.
              </p>
            </div>
