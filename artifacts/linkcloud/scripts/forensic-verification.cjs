/**
 * LinkCloud Forensic Verification & Red Team Audit Script (CommonJS)
...
async function runForensicAudit() {
  ...
  const webmasterWriteLocked = rulesContent.includes("match /webmaster/") && rulesContent.includes("allow write: if false;");
  ...
  const countersLocked = rulesContent.includes("match /counters/") && rulesContent.includes("allow write: if false;");
  ...
  const registryLocked = rulesContent.includes("match /accountUidRegistry/") && rulesContent.includes("allow write: if false;");
  ...
  // Check /auditLogs
  const auditLogsProtected = rulesContent.includes("match /auditLogs/") && rulesContent.includes("allow update, delete: if isWebmaster();");
  ...
  const roleProtected = rulesContent.includes("(!('role' in incoming()) || incoming().role == existing().role)");
  ...
  const noAutoPromotion = !authLibContent.includes("Auto-promoted initial user") && !authLibContent.includes("upsertUserProfile(uid, {\n      uid,\n      email,\n      role: \"webmaster\"");
  ...
  const serverRequiresAuth = serverAdminContent.includes("verifyWebmasterToken") && serverAdminContent.includes("statusCode: 401") && serverAdminContent.includes("statusCode: 403");
  ...
  record("UID Concurrency", "Atomic Sequential UID Generation", isMonotonicAndUnique, `10 allocations generated 10 unique sequential UIDs: ${Array.from(uniqueUids).join(", ")}`);
